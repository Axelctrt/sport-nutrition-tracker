...
if (failures.length === 0) {
  ...
  if (!runtime.includes('SYNC_PROTOTYPE_DATABASE_VERSION = 18')) {
    fail('le runtime cloud v18 attendu est absent.');
  }
  if (!runtime.includes("'sportpilot-sync-runtime-0.20.0-v16'")) {
  ...
  if (!/CURRENT_DATABASE_VERSION\s*=\s*DATABASE_VERSION_13\b/.test(databaseVersions)) {
    fail('le schéma métier local doit utiliser Dexie v13.');
  }
