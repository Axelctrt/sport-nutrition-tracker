...
if (
  !/DATABASE_VERSION_14\s*=\s*14\s+as\s+const\b/.test(
    databaseVersions,
  ) ||
  !/CURRENT_DATABASE_VERSION\s*=\s*DATABASE_VERSION_14\b/.test(
    databaseVersions,
  ) ||
  ...
  fail('le schéma Dexie v14 attendu est absent ou mal relié.');
}

if (
  !/CURRENT_BACKUP_SCHEMA_VERSION\s*=\s*13\b/.test(
    backupMigrations,
  )
) {
  fail('le format de sauvegarde JSON v13 attendu est absent.');
}
