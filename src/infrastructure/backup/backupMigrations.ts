...
function migrateVersion6ToVersion7(input: BackupHeader): unknown {
  const data = asRecord(input.data) ?? {};
...
function migrateVersion8ToVersion9(input: BackupHeader): unknown {
  const data = asRecord(input.data) ?? {};
...
function migrateVersion9ToVersion10(input: BackupHeader): unknown {
  const data = asRecord(input.data) ?? {};
...
function migrateVersion10ToVersion11(input: BackupHeader): unknown {
  return { ...input, schemaVersion: 11 };
}

function migrateVersion11ToVersion12(input: BackupHeader): unknown {
  const data = asRecord(input.data) ?? {};
  return {
    ...input,
    schemaVersion: 12,
    data: { ...data, coachDecisionMemories: [] },
  };
}
...
export function migrateBackupEnvelope(input: unknown): BackupEnvelope {
  ...
  if (version <= 8) {
    migrated = migrateVersion8ToVersion9(readHeader(migrated));
  }
  if (version <= 9) {
    migrated = migrateVersion9ToVersion10(readHeader(migrated));
  }
  if (version <= 10) {
    migrated = migrateVersion10ToVersion11(readHeader(migrated));
  }
  if (version <= 11) {
