...
async function expectFixture(database: Dexie, fixture: FixtureByTable): Promise<void> {
  ...
  for (const [tableName, expectedRecords] of Object.entries(fixture)) {
    ...
    expect([...actualRecords].sort((left, right) => left.id.localeCompare(right.id))).toEqual(
      [...expectedRecords].sort((left, right) => left.id.localeCompare(right.id)),
    );
  }
}

describe('chaîne de migrations Dexie', () => {
  it('conserve le contenu complet du schéma v1 pendant la montée vers la v12', async () => {
    ...
    try {
      ...
      expect(upgradedDatabase.verno).toBe(DATABASE_VERSION_12);
      ...
      for (const tableName of Object.keys(schemaVersion12).filter(
  ...
  it('conserve toutes les tables et relations après fermeture puis réouverture', async () => {
    const databaseName = createDatabaseName();
  ...
  it('préserve le schéma v12 lors de l’ajout simulé d’une future version 13', async () => {
    const databaseName = createDatabaseName();
