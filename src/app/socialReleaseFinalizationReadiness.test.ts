...
import releaseNotesSource from '@/../RELEASE-NOTES-0.30.0.md?raw';
import {
...
describe('release finalization readiness 0.30.0', () => {
  it('expose la version stable dans le build et les métadonnées', () => {
    expect(__APP_VERSION__).toBe('0.30.0');
    expect(packageSource).toContain('"version": "0.30.0"');
    expect(openFoodFactsProxySource).toContain('SportPilot/0.30.0');
  });
  ...
  it('documente la release et le tag stable', () => {
    expect(releaseNotesSource).toContain('SportPilot 0.30.0');
    expect(releaseNotesSource).toContain('Tag attendu : `v0.30.0`');
    expect(releaseChecklistSource).toContain('release/0.30.0');
    expect(releaseChecklistSource).toContain('Tag annoté `v0.30.0`');
  });
