...
import releaseNotesSource from '@/../RELEASE-NOTES-0.36.0.md?raw';
import {
...
describe('release finalization readiness 0.36.0', () => {
  it('expose la version stable dans le build et les métadonnées', () => {
    expect(__APP_VERSION__).toBe('0.36.0');
    expect(packageSource).toContain('"version": "0.36.0"');
    expect(openFoodFactsProxySource).toContain('SportPilot/0.36.0');
  });
  ...
  it('documente la branche UX sans annoncer de publication', () => {
    expect(releaseNotesSource).toContain('SportPilot 0.36.0');
    expect(releaseNotesSource).toContain('Branche : `feat/friends-settings-strength-ux-0.36.0`');
    expect(releaseNotesSource).toContain('Aucun tag ni déploiement');
    expect(releaseChecklistSource).toContain('Branche `feat/friends-settings-strength-ux-0.36.0` créée');
