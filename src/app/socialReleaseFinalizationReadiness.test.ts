...
import releaseNotesSource from '@/../RELEASE-NOTES-1.0.0.md?raw';
import {
...
describe('stable readiness 1.0.0', () => {
  ...
  it('documente la branche stable sans annoncer de publication', () => {
    expect(releaseNotesSource).toContain('SportPilot 1.0.0 — stable en préparation');
    expect(releaseNotesSource).toContain('Branche : `codex/163-release-1-0-0`');
    expect(releaseNotesSource).toContain('non publiée, non taguée et non déployée');
    expect(releaseChecklistSource).toContain('SportPilot 1.0.0');
