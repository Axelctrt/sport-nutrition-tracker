...
export function DashboardPage() {
  ...
  const renderWidget = (widgetId: DashboardWidgetId) => {
    if (!snapshot || !nutrition || !isDashboardWidgetVisible(preferences, widgetId)) return null;

    switch (widgetId) {
      case 'activeWorkout':
        return activeWorkout ? <DashboardActiveWorkout key={widgetId} workout={activeWorkout} /> : null;
      case 'todaySummary':
        return (
      ...
      case 'activities':
        return <DashboardActivities key={widgetId} activities={snapshot.activities} date={date} />;
      case 'calculationDetails':
        return <DashboardCalculationDetails key={widgetId} snapshot={snapshot} />;
    }
  };
  ...
  return (
    <section aria-labelledby="dashboard-title">
      ...
      {preferencesError ? (
        <InlineNotice className="mt-4" tone="error" title="Affichage personnalisé indisponible">
          {preferencesError} L’ordre équilibré est utilisé temporairement.
        </InlineNotice>
      ) : null}
