...
type MetricKind = 'weight' | 'activity' | 'nutrition' | 'strength';
...
function weightDetail(weight: ProgressionHubSummary['weight']): string {
  ...
  if (weight.state === 'insufficient') return 'Une seconde période est nécessaire pour calculer une évolution.';
  if (weight.changeKg === undefined) return 'Moyenne de poids sur la dernière période suivie.';
  return `${signed(weight.changeKg, 'kg')} depuis la période précédente.`;
}

function nutritionValue(nutrition: ProgressionHubSummary['nutrition']): string {
  if (nutrition.averageCaloriesKcal === undefined) return 'À renseigner';
  return `${Math.round(nutrition.averageCaloriesKcal).toLocaleString('fr-FR')} kcal/j`;
}

function nutritionDetail(nutrition: ProgressionHubSummary['nutrition']): string {
  if (nutrition.trackedDays === 0) {
    return 'Ajoute des aliments pour obtenir une moyenne quotidienne.';
  }
  const tracked = `${nutrition.trackedDays} jour${nutrition.trackedDays > 1 ? 's' : ''} suivi${nutrition.trackedDays > 1 ? 's' : ''}`;
  if (nutrition.averageTargetCaloriesKcal === undefined) return `${tracked}.`;
  return `${tracked} · cible journalière moyenne : ${Math.round(nutrition.averageTargetCaloriesKcal).toLocaleString('fr-FR')} kcal.`;
}

function strengthDetail(strength: ProgressionHubSummary['strength']): string {
  const exercise = strength.exerciseName ?? 'Exercice suivi';
  if (strength.changePercent === undefined) return `1RM estimé · ${exercise} · évolution encore indisponible.`;
  if (Math.abs(strength.changePercent) < 0.05) return `1RM estimé · ${exercise} · évolution stable sur la période.`;
  return `1RM estimé · ${exercise} · évolution ${signed(strength.changePercent, '%')} sur la période.`;
}
...
function SparseMetricVisual({
  ...
  if (values.length === 1) {
    ...
    const maximum = Math.max(1, current, referenceValue ?? 0);
    return (
      <div
        role="img"
        aria-label={`${label}. Une mesure disponible.`}
        data-metric-kind={kind}
        data-metric-density="single"
        className="mt-3 rounded-md border border-[var(--sp-border-subtle)] bg-[var(--sp-surface-muted)] px-3 py-2.5"
      >
        <div className="flex items-center justify-between gap-3 text-xs font-semibold text-[var(--sp-text-muted)]">
          <span>1 mesure</span>
          <span>Repère initial</span>
        </div>
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-[var(--sp-surface-elevated)]">
          <span
            className="block h-full rounded-full bg-[var(--sp-chart-1)]"
            style={{ width: `${Math.max(8, current / maximum * 100)}%` }}
          />
        </div>
        {referenceValue !== undefined ? (
          <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[var(--sp-surface-elevated)]" aria-hidden="true">
            <span
              className="block h-full rounded-full bg-[var(--sp-chart-target)]"
              style={{ width: `${Math.max(8, referenceValue / maximum * 100)}%` }}
            />
          </div>
        ) : null}
      </div>
  ...
  return (
    <div data-metric-kind={kind} data-metric-density={values.length <= 3 ? 'sparse' : 'normal'}>
      <SportPilotMiniChart values={values} label={label} />
    </div>
  );
}
...
function MetricCard({
  ...
  return (
    <Card padding="md" className="min-w-0 border-[var(--sp-border-subtle)]" data-metric-card={kind}>
...
function ReviewSummary({ review }: { review: ProgressionHubSummary['review'] }) {
  ...
  return (
    <Card padding="md" aria-labelledby="progression-review-title">
      <p className="text-xs font-bold uppercase text-[var(--sp-accent-primary)]">Bilan hebdomadaire</p>
      <h2 id="progression-review-title" className="mt-1 text-base font-bold text-[var(--sp-text-primary)]">{title}</h2>
...
function GoalSummary({ goal }: { goal: ProgressionHubSummary['goal'] }) {
  ...
  return (
    <Card padding="md" aria-labelledby="progression-goal-title">
      <p className="text-xs font-bold uppercase text-[var(--sp-accent-primary)]">Objectif</p>
      <h2 id="progression-goal-title" className="mt-1 text-base font-bold text-[var(--sp-text-primary)]">{title}</h2>
      <p className="mt-1 text-sm leading-5 text-[var(--sp-text-secondary)]">{detail}</p>
      <Link to={routePaths.goals} className="mt-3 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-[var(--sp-accent-primary)]">
...
export function ProgressionHubPage() {
  ...
  const rangeLabel = progressionRanges.find(({ id }) => id === range)?.label ?? '30 jours';
  ...
  return (
    <section aria-labelledby="progression-hub-title" className="min-w-0">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <h1 id="progression-hub-title" className="text-3xl font-bold text-[var(--sp-text-primary)]">Progression</h1>
      ...
      {summary.status === 'loading' || !summary.data ? (
        <div className="mt-6 grid gap-4 sm:grid-cols-2" aria-label="Chargement de la progression">
          {Array.from({ length: 4 }, (_, index) => (
            <div key={index} className="h-44 animate-pulse rounded-[var(--sp-radius-card)] bg-[var(--sp-surface-muted)] motion-reduce:animate-none" />
          ))}
        </div>
      ) : (
        <div className="mt-6 space-y-6">
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase text-[var(--sp-text-muted)]">Signal principal</p>
                <h2 id="progression-main-signal" className="mt-1 text-lg font-bold text-[var(--sp-text-primary)]">{summary.data.signal.title}</h2>
                <p className="mt-1 text-sm leading-6 text-[var(--sp-text-secondary)]">{summary.data.signal.detail}</p>
                <Link to={destinationPaths[summary.data.signal.destination]} className="mt-2 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-[var(--sp-accent-primary)]">
                  Voir le détail
                  <ArrowRight aria-hidden="true" className="size-4" />
                </Link>
              </div>
          ...
          <section aria-labelledby="progression-overview-title">
            <div className="mb-3 flex items-end justify-between gap-3">
              <div>
                <h2 id="progression-overview-title" className="text-lg font-bold text-[var(--sp-text-primary)]">Vue d’ensemble</h2>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <MetricCard
                ...
                chartLabel={`Évolution des moyennes de poids sur ${rangeLabel}`}
              ...
              <MetricCard
                ...
                chartLabel={`Minutes d’activité sur ${rangeLabel}`}
              ...
              <MetricCard
                ...
                chartLabel={`Calories moyennes par jour sur ${rangeLabel}`}
                to={`${routePaths.analytics}?tab=nutrition`}
                icon={Utensils}
                kind="nutrition"
                {...(summary.data.nutrition.averageTargetCaloriesKcal === undefined
                  ? {}
                  : { referenceValue: summary.data.nutrition.averageTargetCaloriesKcal })}
              />
              {summary.data.strength.state === 'ready' ? (
                <MetricCard
                  ...
                  detail={strengthDetail(summary.data.strength)}
                  values={summary.data.series.strength.slice(-visibleWeeks)}
                  chartLabel={`Évolution du 1RM estimé pour ${summary.data.strength.exerciseName ?? 'l’exercice'}`}
          ...
          <nav aria-labelledby="progression-domains-title">
            <div className="mb-3">
              <h2 id="progression-domains-title" className="text-lg font-bold text-[var(--sp-text-primary)]">Tes domaines</h2>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                return (
                  <Link key={domain.label} to={domain.path} className="sp-card sp-card--interactive flex min-h-12 items-center justify-between gap-3 px-3 text-sm font-bold text-[var(--sp-text-primary)]">
          ...
          <section aria-labelledby="progression-week-title">
            <div className="mb-3">
              <h2 id="progression-week-title" className="text-lg font-bold text-[var(--sp-text-primary)]">Cette semaine</h2>
            ...
            <Card padding="none" className="overflow-hidden">
              <dl className="grid gap-px bg-[var(--sp-border-subtle)] sm:grid-cols-2 lg:grid-cols-4">
                {[
                  { label: 'Activités réalisées / prévues', value: `${summary.data.week.completedActivities} / ${summary.data.week.plannedActivities}`, icon: CalendarCheck2 },
                  { label: 'Repos confirmés', value: summary.data.week.confirmedRestDays.toString(), icon: CheckCircle2 },
                  { label: 'Jours avec check-in', value: summary.data.week.checkInDays.toString(), icon: Activity },
                  { label: 'Jours avec nutrition', value: summary.data.week.nutritionDays.toString(), icon: Utensils },
                ].map((item) => {
                  ...
                  return (
                    <div key={item.label} className="bg-[var(--sp-surface-card)] p-4">
                      ...
                      <dd className="mt-2 text-xl font-bold tabular-nums text-[var(--sp-text-primary)]">{item.value}</dd>
                    </div>
                  );
                })}
              </dl>
          ...
          <nav aria-label="Ressources de progression" className="border-t border-[var(--sp-border-subtle)] pt-5">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                return (
                  <Link key={destination.path} to={destination.path} className="sp-card sp-card--interactive flex min-h-12 items-center gap-3 px-3 text-sm font-bold text-[var(--sp-text-primary)]">
                    <Icon aria-hidden="true" className="size-4 text-[var(--sp-accent-primary)]" />
                    {destination.label}
                  </Link>
                );
