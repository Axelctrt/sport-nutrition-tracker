    ...
    <form noValidate onSubmit={handleSubmit(onSubmit)} className="space-y-7">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <FormField id="strength-exercise-primary-muscle" label="Groupe musculaire principal" error={errors.primaryMuscleGroup?.message} required>
          <select id="strength-exercise-primary-muscle" className={inputClassName} {...register('primaryMuscleGroup')}>
        ...
        <FormField id="strength-exercise-equipment" label="Matériel" error={errors.equipment?.message} required>
          <select id="strength-exercise-equipment" className={inputClassName} {...register('equipment')}>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <FormField id="strength-exercise-category" label="Catégorie" error={errors.category?.message} required>
          <select id="strength-exercise-category" className={inputClassName} {...register('category')}>
        ...
        <FormField id="strength-exercise-movement" label="Type de mouvement" error={errors.movementType?.message} required>
          <select id="strength-exercise-movement" className={inputClassName} {...register('movementType')}>
      ...
      <FormField id="strength-exercise-load-unit" label="Unité de charge" error={errors.loadUnit?.message} required>
        <select id="strength-exercise-load-unit" className={inputClassName} {...register('loadUnit')}>
          {loadUnitOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
        </select>
      </FormField>

      <FormField id="strength-exercise-description" label="Description" description="Facultatif. Indique les repères techniques utiles." error={errors.description?.message}>
        <textarea id="strength-exercise-description" rows={5} className={inputClassName} {...register('description')} />
      </FormField>
