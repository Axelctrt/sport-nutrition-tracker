...
export function isLocalDevelopmentOrigin(locationValue: Location = window.location): boolean {
 ...
 * ni IndexedDB ni localStorage et ne s'exécute que sur localhost/127.0.0.1.
 */
export async function cleanupLocalDevelopmentPwa(): Promise<boolean> {
  if (!isLocalDevelopmentOrigin()) return false;

  const wasControlled = 'serviceWorker' in navigator
  ...
  if (wasControlled && sessionStorage.getItem(RELOAD_GUARD_KEY) !== 'done') {
